...
class Image:
    ...
    def _update_scaled_image(self):
        ...
        if self.size_mode == "auto":
            ...
            final_w, int(original_w * ratio)
            final_h = int(original_h * ratio)
    ...
    def draw(self, surface):
        ...
        image_center_offset = [self.image.get_width() / 2, self.image.get_height() / 2]

        if self.image_center:
            draw_pos = [
                int(self.pos[0] + offset[0] - image_center_offset[0]),
    ...
    def collide_other(self, other):
        if self.image is None or other.image is None:
            return False
        my_rect = self.image.get_rect(topleft=(self.pos[0] + self.game_center[0], self.pos[1] + self.game_center[1]))
        other_rect = other.image.get_rect(topleft=(other.pos[0] + other.game_center[0], other.pos[1] + other.game_center[1]))
