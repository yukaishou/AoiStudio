...
class CFGDecoder:
    ...
    def wait(self, args):
        """等待指定秒数（非阻塞）"""
        if not args:
            print("wait 指令缺少时间参数，用法：wait 1.5")
            return
        try:
            duration = float(args[0])
            if duration < 0:
                print(f"等待时间不能为负数：{duration}")
                return
            self.wait_until = pygame.time.get_ticks() + duration * 1000
            self.is_waiting = True
            print(f"开始等待 {duration} 秒...")
        except ValueError:
            print(f"无效的等待时间：{args[0]}，请输入数字（如 1.0）")
    ...
    def add_object(self, args):
        if not args:
            print("add 指令缺少类型参数（character/background/flag）")
            return
        obj_type = args[0].lower()

        if obj_type == "character":
            if len(args) < 4:
                print("add character 用法：add character file:xxx.png x y")
                return
            try:
                path = args[1]
                x = float(args[2])
                y = float(args[3])
                print(f"添加角色：路径={path}, 位置=({x}, {y})")
            ...
            if len(args) < 2:
                print("add background 用法：add background file:xxx.png")
                return
            path = args[1]
            print(f"添加背景：路径={path}")
            if path.startswith("file:") and self.engine:
                self.engine.scene.add_background(path[5:])

        elif obj_type == "flag":
            if len(args) < 2:
                print("add flag 用法：add flag flag_name")
                return
            flag_name = args[1]
            print(f"添加标志：{flag_name}")
            if self.engine:
                self.engine.dialog.add_flag(flag_name)

        else:
            print(f"未知 add 对象类型：{obj_type}")

    def affection_control(self, args):
        if len(args) < 3:
            print("affection 用法：affection 角色名 add/set/reduce 数值")
            return
        ...
        try:
            ...
            if op == "add":
                ...
                print(f"未知 affection 操作：{op}")
            print(f"{char_name} 好感度 {op} {val}")
        except ValueError:
            print(f"好感度数值无效：{val_str}")

    def move_object(self, args):
        if len(args) < 6:
            print("move 用法：move character 索引 x y 缓动 时长")
            return
        try:
            obj_type, idx_str, x_str, y_str, ease, dur_str = args[:6]
            if obj_type != "character":
                print(f"仅支持 move character，不支持 {obj_type}")
                return
            idx = int(idx_str)
            x, y = float(x_str), float(y_str)
            dur = float(dur_str)
            print(f"角色 {idx} 移动到 ({x}, {y})，缓动={ease}，时长={dur}s")
    ...
    def animation(self, args):
        if len(args) < 3:
            print("animation 用法：animation character 索引 动画名 ...")
            return
        try:
            obj_type, idx_str, anim = args[:3]
            if obj_type != "character":
                print(f"仅支持 animation character，不支持 {obj_type}")
                return
            idx = int(idx_str)
            print(f"角色 {idx} 执行动画：{anim}")
            ...
            if anim == "fade_to" and len(args) >= 5:
                ...
                print(f"未知动画或参数不足：{anim}")
        except (ValueError, IndexError) as e:
            print(f"animation 参数错误：{e}")

    def switch(self, args):
        if not args:
            print("switch 指令缺少类型（background/bgm/character）")
            return
        target = args[0].lower()
        ...
        if target == "background" and len(args) >= 4:
            ...
            try:
                ...
                print(f"switch character 参数错误：{e}")

        else:
            print(f"switch 用法错误或参数不足：{' '.join(args)}")
